...
public class FilialDAO {
	...
	public void create(Filial filial) throws DAOException {
		try (Connection con = Database.getInstance().getConnection()) {
			...
			PreparedStatement stmt = con.prepareStatement(INSERTFILIAL, PreparedStatement.RETURN_GENERATED_KEYS);
			...
			PreparedStatement stmt2 = con.prepareStatement(INSERTENDERECO);
	...
	public void update(Filial filial) throws DAOException {
		
		try (Connection con = Database.getInstance().getConnection()) {
			PreparedStatement stmt = con.prepareStatement(UPDATEFILIAL);
				stmt.setString(1, filial.getNome());
				stmt.setString(2, filial.getCnpj());
				stmt.setString(3, filial.getInscEstadual());
			...
			PreparedStatement stmt2 = con.prepareStatement(UPDATEENDERECO);
				stmt2.setString(1, filial.getEndereco().getRua());
				stmt2.setString(2, filial.getEndereco().getNumero());
				stmt2.setString(3, filial.getEndereco().getBairro());
	...
	public List<Filial> getFiliais() throws DAOException {
	...
	public Filial getFilial(int id) throws DAOException{
		Filial filial = null;
		try (Connection con = Database.getInstance().getConnection()){
			PreparedStatement stmt = con.prepareStatement(LISTBYID);
				...
				String bairro = rs.getString("bairro");
				filial = new Filial(nome, cnpj, inscEstadual, new Endereco(rua, numero, bairro));
